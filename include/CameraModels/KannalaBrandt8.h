...
namespace ORB_SLAM3 {
    ...
    public:
        ...
        KannalaBrandt8(std::shared_ptr<KannalaBrandt8> pKannala) : GeometricCamera(pKannala->mvParameters), mvLappingArea(2,0), precision(pKannala->precision) ,tvr(nullptr) {
            assert(mvParameters.size() == 8);
            mnId=nNextId++;
            mnType = CAM_FISHEYE;
        }
        ...
        float TriangulateMatches(std::shared_ptr<GeometricCamera> pCamera2, const KeyPoint& kp1, const KeyPoint& kp2,  const Eigen::Matrix3f& R12, const Eigen::Vector3f& t12, const float sigmaLevel, const float unc, Eigen::Vector3f& p3D);
