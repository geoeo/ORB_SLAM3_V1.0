    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(getTrackingState()==OK)
            {
                ...
                if((!mbVelocity && !mpAtlas->GetCurrentMap()->isImuInitialized()) || mCurrentFrame.mnId<mnLastRelocFrameId+2)
                {
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if (((mnMatchesInliers<100) || getTrackingState()==RECENTLY_LOST)) // MODIFICATION_2, originally ((((mnMatchesInliers<75) && (mnMatchesInliers>15)) || mState==RECENTLY_LOST) && ((mSensor == System::IMU_MONOCULAR)))
        c4=true;

    Verbose::PrintMess("NeedNewKeyFrame: c1a " + to_string(c1a) + " c1b " + to_string(c1b) + " c1c " + to_string(c1c) + " c2 " + to_string(c2) + " c3 " + to_string(c3) + " c4 " + to_string(c4), Verbose::VERBOSITY_NORMAL);
    if(((c1a||c1c) && c2)|| c3 || c4)
    {
...
void Tracking::ResetActiveMap(bool bLocMap)
{
    ...
    int num_lost = 0;
    Verbose::PrintMess("mnInitialFrameId = " + to_string(mnInitialFrameId), Verbose::VERBOSITY_NORMAL);
    ...
    Verbose::PrintMess(to_string(num_lost) + " Frames set to lost", Verbose::VERBOSITY_NORMAL);
