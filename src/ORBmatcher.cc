...
namespace ORB_SLAM3
{
                ...
                for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
                {
                    ...
                    if(bestDist1<=TH_LOW)
                    {
                        if(static_cast<float>(bestDist1)<mfNNratio*static_cast<float>(bestDist2))
                        {
                            ...
                            const auto &kp = pKF->mvKeysUn[realIdxKF];

                            if(mbCheckOrientation)
                            {
                                auto &Fkp = F.mvKeys[bestIdxF];
        ...
        while(f1it!=f1end && f2it!=f2end)
        {
            if(f1it->first == f2it->first)
            {
                for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
                {
                    ...
                    const auto &kp1 = pKF1->mvKeysUn[idx1];
                    ...
                    for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                    {
                        ...
                        const auto &kp2 = pKF2->mvKeysUn[idx2];
                    ...
                    if(bestIdx2>=0)
                    {
                        const auto &kp2 = pKF2->mvKeysUn[bestIdx2];
    ...
    int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th, const bool bRight)
    {
        ...
        for(int i=0; i<nMPs; i++)
        {
            ...
            {
                size_t idx = *vit;
                const auto &kp = pKF->mvKeysUn[idx];
    ...
    int ORBmatcher::SearchBySim3(KeyFrame* pKF1, KeyFrame* pKF2, std::vector<MapPoint *> &vpMatches12, const Sophus::Sim3f &S12, const float th)
    {
        ...
        for(int i1=0; i1<N1; i1++)
        {
            ...
            for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
            {
                ...
                const auto &kp = pKF2->mvKeysUn[idx];
        ...
        for(int i2=0; i2<N2; i2++)
        {
            ...
            for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
            {
                ...
                const auto &kp = pKF1->mvKeysUn[idx];
            ...
            if(pMP)
            {
                if(!LastFrame.mvbOutlier[i])
                {
                    ...
                    if(bestDist<=TH_HIGH)
                    {
                        ...
                        if(mbCheckOrientation)
                        {
                            auto kpLF = LastFrame.mvKeysUn[i];

                            auto kpCF = CurrentFrame.mvKeysUn[bestIdx2];
