...
MapPoint::MapPoint():
    ...
    mpReplaced(nullptr), mWorldPos(Eigen::Vector3f::Zero()), mGNSSPos(std::nullopt)
{
    mpReplaced = nullptr;
}
...
void MapPoint::SetGNSSPos(const Eigen::Vector3d &pos) {
    unique_lock<mutex> lock(mMutexPos);
    mGNSSPos = pos;
}
...
{
    unique_lock<mutex> lock(mMutexPos);
    return 0.9f * mfMinDistance;
}

float MapPoint::GetMaxDistanceInvariance()
{
    unique_lock<mutex> lock(mMutexPos);
    return 1.1f* mfMaxDistance;
}
...
int MapPoint::PredictScale(const float &currentDist, shared_ptr<Frame> pF)
{
    float ratio;
    {
        unique_lock<mutex> lock(mMutexPos);
